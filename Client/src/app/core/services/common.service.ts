...
export class CommonService {
    ...
    stringFormat(inputString: string, ...args: any[]) {
    ...
    addWorkDays(date: Date, days: number) {
        const cloned = new Date(date);
        while (days > 0) {
    ...
    handleError(error: any, injector: Injector) {
        ...
        if (error.status === 401) {
            const authenService = injector.get(AuthService);
